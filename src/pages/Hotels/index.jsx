...
//             currentPage === 1 ? 'bg-white border border-teal-500 text-gray-700' : 'bg-teal-500 text-white hover:bg-teal-700'
//           }`}
...
//             className={`px-4 py-2 mx-1 rounded ${
//               currentPage === index + 1 ? 'border border-teal-500 bg-teal-500 text-white ' : 'bg-white text-gray-700 border hover:bg-teal-500 hover:bg-opacity-30  hover:black'
//             }`}
//           >
//             {index + 1}
//           </button>
...
//         <button
//           className={`px-4 py-2 mx-1 rounded ${
//             currentPage === totalPages ? 'bg-white border border-teal-500 text-gray-700 ' : 'bg-teal-500 text-white hover:bg-teal-700 '
//           }`}
//           onClick={() => setCurrentPage(currentPage + 1)}
...
import hotelsData from '../../data/hotelsData.json';
import BookHotelWrapper from './BookHotelWrapper';
import { userLoginContext } from '../../context/userLogoncontext'; // Adjust path as per your file structure
...
const Hotels = () => {
  ...
  const { userstatus } = useContext(userLoginContext);
  ...
  const filterHotels = (hotel) => {
    ...
    return (
      ...
      hotel.hotelAddress.toLowerCase().includes(term) ||
      hotel.prices.some((priceInfo) =>
        priceInfo.roomType.toLowerCase().includes(term) ||
        priceInfo.price.toString().includes(term)
      )
    );
  };
  ...
  return (
    <>
      ...
      <div className='p-6 md:flex lg:flex bg-gray-100 gap-4'>
        ...
        <div className='grid sm:grid md:flex lg:flex flex-col md:w-8/12 lg:w-8/12 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
          <div className='addSearch w-full flex items-center justify-center'>
            <input
              type='text'
              placeholder='Search hotels...'
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className='px-4 py-2 rounded-lg bg-white border-2 border-teal-500 focus:outline-none focus:shadow-xl transition-all duration-500 focus:transition-all focus:duration-500  focus:border-teal-600 w-6/12'
            />
          </div>
      ...
      {showLoginPrompt && (
        <div className='fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-gray-900 bg-opacity-50'>
          <div className='bg-white p-8 rounded-lg shadow-lg flex flex-col justify-center items-center '>
            <p className='text-lg text-gray-800 mb-4'>You are not logged in till now</p>
            <NavLink to='../login'
              className='bg-teal-500 px-4 py-2 mt-4 text-white rounded hover:bg-teal-700'
